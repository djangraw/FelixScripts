function data = ImportSartData(filename)

% Created 1/5/15 by DJ. 

if nargin==0
    filename = 'Sart-1-1-DEMO.log'; % demo
end
...
% get performance
iTrials = find(ismember(events.display.name,{'Target','NonTarget'}));
...
for i=1:nTrials
    performance.isTarget(i) = strcmp(events.display.name(iTrials(i)),'Target');
    if i<nTrials

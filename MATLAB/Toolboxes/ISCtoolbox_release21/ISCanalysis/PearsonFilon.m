...
%    mMapmatResultWhole.([Priv.prefixFreqComp num2str(freqBlock(fr))]).Writable = true
    if ( mMapmatResultWhole.([Priv.prefixFreqComp num2str(freqBlock(fr))]).Writable == false ) %...
       % || ( mMapResultWhole.([Priv.prefixFreqComp num2str(freqBlock(fr))]).Writable == false ) 
        ...
        save([Pub.dataDestination '/tmp/rerun_' num2str(fr)],'fr')
        return
    end
end
...
Z2 = zeros([Priv.dataSize(nrSession,1),Priv.dataSize(nrSession,2),Priv.dataSize(nrSession,3),length(freqBlock),nrSubjPairs], 'single');
                ...
                if bmask(xx,yy,zz)
                    ...
                    if wfr == 1
                        ...
                        for freqbInd = 1:length(freqBlock)
                            ...
                            Z2(xx,yy,zz,freqbInd,:) = single(VAL);
...
%clear memMaps
save([Pub.dataDestination '/tmp/' num2str(nrSession) '_' num2str(freqBlock) '_memMaps'],'freqBlock')
showTime(0);

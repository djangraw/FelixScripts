...
switch(spm_ver),
    ...
    otherwise, disp(['Warning! unrecognized SPM version ',spm_ver]); spm_ver=12;
end
...
if ~isempty(sess_file) % read config
    [num_sess,global_type_flag,drop_flag,gui_display,motionFileType,motion_threshold,global_threshold,use_diff_motion,use_diff_global,use_norms,SPMfile,mask_file,output_dir,P,M] = ...
...
if option>2
    % computes Var/SNR
    if numel(out_idx)>100, hw=waitbar(0,'updating Variance/SNR plots. Please wait'); end
    N=numel(plotdata.VY);
    ...
    for n1=1:numel(out_idx),
        ...
        Data_Sum=Data_Sum-temp;
        Data_SumSquared=Data_SumSquared-temp.^2;
        if numel(out_idx)>100, waitbar(n1/numel(out_idx),hw); end
    end
    ...
    if numel(out_idx)>100, close(hw); end
end
...
for noption=1:numel(option)
    ...
    spm_write_vol(V,b);
    disp(['New ',filedescription,' file saved to ',V.fname]);
end
end
...
% ohinds 2008-04-23
% -----------------------------------------------------------------------
function [num_sess,global_type_flag,drop_flag,gui_display,motionFileType,motion_threshold,global_threshold,use_diff_motion,use_diff_global,use_norms,SPMfile,mask_file,output_dir,P,M] = read_art_sess_file(sess_file)
...
if (isempty(SPM))
    ...
    switch(spm_ver),
        ...
        case {'SPM8b','SPM8'}, spm_ver=8;
        case {'SPM12b','SPM12'}, spm_ver=12;
        otherwise, disp(['Warning! unrecognized SPM version ',spm_ver]); spm_ver=8;
    end
    switch spm_ver
        case {1,2}

...
options=struct('breakconditionsbysession',false,'deleteall',false);
...
if ~nargin||isempty(filename),
    ...
    answ=conn_questdlg('','Condition file format',fileoptions{[1:numel(fileoptions) 1]},'tooltipstring',tooltipstrings{:});
        ...
        if filetype==3
            ...
            for nsub=1:CONN_x.Setup.nsubjects
                ...
                for nses=1:nsess
                    ...
                    filename{nsub}{nses}=fullfile(cwd,f{nf});
                    fprintf('Subject %d Session %d: %s\n',nsub,nses,filename{nsub}{nses});
                end
            end
        end
        conditions={}; onsets=[]; durations=[]; nsess=[]; nsubs=[];
        for nsub=1:numel(filename)
            if ~iscell(filename{nsub}), filename{nsub}={filename{nsub}}; allsess=true;
            else allsess=false; 
            end
            for nses=1:numel(filename{nsub})
                [tonsets,tdurations,tconditions]=conn_importcondition_readbids(filename{nsub}{nses});

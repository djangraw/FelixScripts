function updated=conn_update(thisver,force,silent)
global CONN_x
...
if ~ok, if ~silent, conn_update_disp(['Cannot access ',[url0,url1],' to check for updates. Please try again later'],2); end
    ...
    if force||existprev,
        ...
        if samever,
            ...
            if 0,% only check / do not offer to download 
                conn_update_disp({txt,['Update (',version_url{1},'.',version_url{2},') available at ',url0,url1]});
            else,
                if ~silent, conn_update_disp({txt,['Update (',version_url{1},'.',version_url{2},') available at ',url0,url1]}); end
                if force==2, answ='Yes'; 
                else answ=questdlg({['Update (',version_url{1},'.',version_url{2},') available at ',url0,url1],'Download and install this update now?'},'conn update','Yes', 'No', 'No');
                end
                if strcmp(answ,'Yes'),
...
%                         ok=0;
                        ...
                        if ~isequal(path1,0),
                            ...
                            conn_update_disp('Downloading updates. Please wait...');
                            unzip([url0,url3],path1);
                            
                            conn_update_disp({['CONN successfully updated (release ',version_url{1},'.',version_url{2},')'],['Installation folder ',path2]},1);
                            try
        ...
        conn_update_disp({['CONN version ',version_url{1},'.',version_url{2},' available at ',url0,url1]},1);
    end
end

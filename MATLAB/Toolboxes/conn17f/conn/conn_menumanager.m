                ...
                if strcmp(handle,'cursorup'),
                    ...
                    if  ~isempty(CONN_MM.MENU{n0}.callback{n1}),
                        if iscell(CONN_MM.MENU{n0}.callback{n1}) && CONN_MM.MENU{n0}.state(n1), 
                            ...
                            set(CONN_MM.gcf,'pointer','watch');pause(.001);
                ...
                if ~strcmp(handle,'cursormove'),
                    for n0=reshape(setdiff(1:numel(CONN_MM.onregioncallback),[md,CONN_MM.onregioninside]),1,[]),
                        if all(ishandle(CONN_MM.onregioncallback{n0}))
                            if CONN_MM.onregionvisible(n0)>0&&all(strcmp(get(CONN_MM.onregioncallback{n0},'visible'),'on')), offregions=[offregions CONN_MM.onregionhandle{n0}];
                            elseif CONN_MM.onregionvisible(n0)<0&&~all(strcmp(get(CONN_MM.onregioncallback{n0},'visible'),'on')), onregions=[onregions CONN_MM.onregionhandle{n0}];
                            end
                        end
                    end
                end
                    ...
                    for n0=CONN_MM.onregioninside(~any(match,2)'),%reshape(setdiff(CONN_MM.onregioninside,md),1,[]) %out
                        if n0>0
                            ...
                            if cond&&all(ishandle(CONN_MM.onregionhandle{n0})),
                                if CONN_MM.onregionvisible(n0)>0, offregions=[offregions CONN_MM.onregionhandle{n0}]; %set(CONN_MM.onregionhandle{n0},'visible','off');
                                elseif CONN_MM.onregionvisible(n0)<0, onregions=[onregions CONN_MM.onregionhandle{n0}]; %set(CONN_MM.onregionhandle{n0},'visible','on');
...
%                     for n0=md,%setdiff(md,CONN_MM.onregioninside) %in
                    for n0=md(~any(match,1)),%reshape(setdiff(md,CONN_MM.onregioninside),1,[]) %in
                        ...
                        if cond&&all(ishandle(CONN_MM.onregionhandle{n0})),
                            if CONN_MM.onregionvisible(n0)>0, onregions=[onregions CONN_MM.onregionhandle{n0}]; %set(CONN_MM.onregionhandle{n0},'visible','on');
                            elseif CONN_MM.onregionvisible(n0)<0, offregions=[offregions CONN_MM.onregionhandle{n0}]; %set(CONN_MM.onregionhandle{n0},'visible','off');
                ...
                if ~isempty(offregions), 
                    set(offregions,'visible','off'); end
                ...
                if ~isempty(colorB), for n1=1:numel(colorB), set(colorB(n1),'foregroundcolor',[.4 .4 .4]+.2*(mean(get(colorB(n1),'backgroundcolor'))<.5)); end; end
                if ~isempty(colorC), for n1=1:numel(colorC), set(colorC(n1),'foregroundcolor',[0 0 0]+1*(mean(get(colorC(n1),'backgroundcolor'))<.5)); end; end
...
%                 end
		...
		case 'off',
 		...
 		case 'clf',
            ...
            if ~isfield(CONN_gui,'isresizing')||~CONN_gui.isresizing
                hax=axes('units','norm','position',[.91,.91,.07,.02],'parent',CONN_MM.gcf); text(0,0,'please wait...','horizontalalignment','center','color',CONN_gui.fontcolorA,'parent',hax); set(hax,'xlim',[-1 1],'ylim',[-1 1],'visible','off');
                [nill,hc]=conn_menu_plotmatrix('',CONN_MM.gcf,[4 1 8],[.91 .93 .07 .02]);
                %hax=axes('units','norm','position',[.2,.025,.10,.02],'parent',CONN_MM.gcf); text(0,0,'please wait...','horizontalalignment','center','color',CONN_gui.fontcolorA,'parent',hax); set(hax,'xlim',[-1 1],'ylim',[-1 1],'visible','off');
                %[nill,hc]=conn_menu_plotmatrix('',CONN_MM.gcf,[4 1 10],[.20 .005 .10 .02]);
    ...
    if isempty(handle), 
        ...
        params=struct(...
            ...
            'color',1/6*.5+.5*[2/6,2/6,6/6; 2/6,2/6,6/6; 2/6,2/6,6/6],...     % 3x3 matrix of color components. Rows are status (cursor-out,cursor-in,selected) and columns are rgb components
        ...
        if strcmp(params.bordertype,'round'), 
            params.color=max(0,min(1, diag([1 1 1])*params.color(:,[3 3 1]) )); 
            %%%params.color(2,:)=params.color(3,:);%(:,[3 2 1]); 
    ...
    for nhandle=1:length(handle),
        ...
        if CONN_MM.MENU{thishandle}.off || CONN_MM.MENU{thishandle}.on
            ...
            for n1=changed(:)',%1:length(CONN_MM.MENU{thishandle}.handle.axes),
				if numel(CONN_MM.MENU{thishandle}.handle.axes)>=n1&&any(ishandle(CONN_MM.MENU{thishandle}.handle.axes{n1})),
					...
					delete(CONN_MM.MENU{thishandle}.handle.axes{n1}(ishandle(CONN_MM.MENU{thishandle}.handle.axes{n1}))); 
				end
            end
        ...
        if CONN_MM.MENU{thishandle}.on,
            ...
            for n1=changed(:)',%1:CONN_MM.MENU{thishandle}.n,
                ...
                if ~CONN_gui.doemphasis3&&CONN_MM.MENU{thishandle}.linkon, ximage=max(0,min(1,ximage*1.15)); end %ximage=ximage.*repmat(.6+.3*tanh(min(0:size(ximage,2)-1,size(ximage,2)-1:-1:0)/10),[size(ximage,1),1,size(ximage,3)]); end %ximage=.5*ximage+.5*mean(CONN_MM.CDATA{thishandle}{n1}{min(3,1+x)});end
                ...
                if length(CONN_MM.MENU{thishandle}.handle.axes)>=n1 && any(ishandle(CONN_MM.MENU{thishandle}.handle.axes{n1})), delete(CONN_MM.MENU{thishandle}.handle.axes{n1}(ishandle(CONN_MM.MENU{thishandle}.handle.axes{n1}))); end
...
%                 end
                if x==3, fontsize=fontsize+CONN_MM.MENU{thishandle}.dfont; 
                ...
                if 1,%CONN_gui.dounixGUIbugfix,
                    ...
                    for n2=1:nstrings
                        ...
                        if CONN_gui.ismac||CONN_MM.MENU{thishandle}.linkon
                            ...
                            htemp=axes('units','norm','position',tpos,'parent',CONN_MM.gcf);
                        end
                        ...
                        if CONN_gui.ismac||CONN_MM.MENU{thishandle}.linkon
                            ...
                            if ~CONN_MM.MENU{thishandle}.enable(n1), set(htemp,'fontangle','italic','foregroundcolor',.5*fontcolor+.5*mean(ximage(:)));
                            else set(htemp,'foregroundcolor',fontcolor);
                            end
                        else
...
switch(lower(params.bordertype))
    case 'round'
%         rns=.5;
%         t=(.1*abs(tx./kx).^rns)+1*abs(ty./ky).^rns;t=max(0,min(1,t));
%         t=t.^.5/2;
%         bg=params.backgroundcolor;
        ...
        t(tx>=kx-2)=0;
        t2=repmat(.5+.5*linspace(1,0,size(t,1))'.^2,1,size(t,2)).*t;
        t1=repmat(.75+.25*linspace(1,0,size(t,1))'.^2,1,size(t,2)).*t;
    ...
    case 'square'
        ...
        elseif strcmp(params.order,'horizontal'), t2=1-.5*repmat(linspace(1,-1,size(t,1))'.^10,1,size(t,2)); t1=.75-.5*repmat(linspace(1,-1,size(t,1))'.^10,1,size(t,2));
        else t2=1-.5*repmat(linspace(1,-1,size(t,2)).^50,size(t,1),1); t1=.75-.5*repmat(linspace(1,-1,size(t,2)).^50,size(t,1),1);
        end
        %t2=.75+.25*repmat(linspace(1,-1,size(t,1))'.^5,1,size(t,2));%t2(ty>=ky-2|tx>=kx-2)=1;
        %t1=.25-0*repmat(linspace(1,-1,size(t,1))'.^5,1,size(t,2));%t2(ty>=ky-2|tx>=kx-2)=1;
        %t2=.5*repmat(linspace(1,-1,size(t,1))'.^6,1,size(t,2)).*t;%t2(ty>=ky-2|tx>=kx-2)=1;
        %t1=.25+.0*repmat(linspace(0,1,size(t,1))'.^6,1,size(t,2)).*t;%t2(ty>=ky-2|tx>=kx-2)=1;
        %if strcmp(params.order,'horizontal'), t1=[t1(1+2:end,:);.5*ones(2,size(t,2));]; end
        %if strcmp(params.order,'vertical'), t2=[.5*ones(2,size(t,2));t2(1:end-2,:)]; end
        %if strcmp(params.order,'horizontal'), t2=[zeros(4,size(t,2));t2(1:end-4,:)]; end
        %if strcmp(params.order,'horizontal'), t1(end-1:end,:)=1; else t1(:,1:2)=0; end
        %t1(ty/ky>=.95|ty/ky<=.05)=0;
        bg=params.backgroundcolor;
        %t={t1,.8*t1+.2*t2,t2};
        t={t1,.25*t1+.75*t2,t2};

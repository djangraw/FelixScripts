...
% fast-ICA algorithm (Aapo Hyv?rinen "Fast and Robust Fixed-Point Algorithms for Independent Component Analysis")
%
% [S,W]=CONN_ICA(X,k)
...
if N>Nd, fprintf('conn_ica warning: number of components greater than dimensions of data; estimating %d components instead\n',Nd); N=Nd; end
...
for n=1:N
    ...
    if n<Nd, 
        ...
        if niter==options.maxiter, disp('conn_ica warning: maximum iteration reached');
            %else fprintf('component %d converged at %d iterations\n',n,niter);
        end
    end
    W(n,:)=w';
    P=W(1:n,:);

...
if nargin<5||isempty(thr),    
    if measures==1,
        h=conn_msgbox('computing variable threshold range, please wait...','conn_network');
        switch(normalization)
            case 0,THR=0:.05:.75;
            case 1,THR=0:.1:2;
            case 2,THR=0:.025:.5;
...
%                 C=(t>=st(max(1,round(thr*numel(st)))));
        ...
        for nthr=1:numel(THR),
            thr=THR(nthr);
            [nill,nill,nill,d,c,k]=conn_network_efficiency([N,mean(net_k(:,nthr))],1e1);
            net_c_rand(nthr)=c;
            net_d_rand(nthr)=d;
            net_k_rand(nthr)=k;
        end
        ...
        figure('name','Network theory: explore variable threshold range','numbertitle','off');clf;
        for n1=1:numel(x),
            thr=mean(y{1},1);
            mnet_d=mean(x{n1},1);
            snet_d=std(x{n1},0,1);
        ...
        %THR=linspace(0,.99*min(max(max(ZZ,[],1),[],2),[],3),20);
        net_d=zeros(size(ZZ,3),numel(THR));net_c=net_d;
        for n=1:size(ZZ,3),
            for nthr=1:numel(THR),
                thr=THR(nthr);
                if normalization==2, C=(ZZ(:,:,n)>=1-thr); else C=ZZ(:,:,n)>=thr; end
%                 C=(ZZ(:,:,n)>thr);
                [c,ac,nac]=conn_network_clustering(C);
                [d,ad,nad]=conn_network_mindist(C);
                net_c(n,nthr)=nac;
                net_d(n,nthr)=nad;
            end
        end
        ...
        %plots network measures
        x={net_d,net_c};
        xlabels={{'\fontsize{16}Normalized path','length  \fontsize{11}L/L^{random}'},{'\fontsize{16}Normalized clustering',' coefficient \fontsize{11}C/C^{random}'}};
        figure;clf;
        for n1=1:numel(x),
            mnet_d=mean(x{n1},1);
            snet_d=std(x{n1},0,1);
            ...
            h=patch([THR,fliplr(THR)],[mnet_d+spm_invTcdf(1-alpha/m/2,n-1)*snet_d/sqrt(n),fliplr(mnet_d-spm_invTcdf(1-alpha/m/2,n-1)*snet_d/sqrt(n))],'k');
            set(h,'facecolor',.75*[1,1,1],'edgecolor','none');
            plot(THR,mnet_d,'k-','linewidth',2);
            hold off;
            set(gcf,'color','w');set(gca,'fontsize',16);
            ylabel(xlabels{n1});xlabel('\fontsize{16}threshold');
        end

        if isempty(filename), return; end
        if isnumeric(posstr)&&isempty(findobj(0,'tag','Interactive')), spm('CreateIntWin'); end;
        thr=spm_input('threshold',posstr,'r',mat2str(1),[1,1]);posstr='+1';
        if isempty(thr),return;end
    end
end

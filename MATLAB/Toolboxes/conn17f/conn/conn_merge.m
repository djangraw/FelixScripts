...
if nargin>0&&ischar(value0), % merge multiple project files (value0 = filenames)
    ...
    for n1=1:size(filenames,1),
        fprintf('loading info from project %s\n',filenames{n1});
        other{1+n1}=load(deblank(filenames{n1}),'CONN_x','-mat');
        if isfield(other{1+n1}.CONN_x,'pobj')&&~other{1+n1}.CONN_x.pobj.holdsdata, % .dmat extended projects terminal nodes
            other{1+n1}.CONN_x.filename=conn_fullfile(conn_projectmanager('parentfile',deblank(filenames{n1}),other{1+n1}.CONN_x.pobj));
            ...
            other{1+n1}.CONN_x.filename=deblank(filenames{n1});
            other{1+n1}.CONN_x=conn_updatefolders(other{1+n1}.CONN_x);
...
if mergeinfo
    ...
    for nsub=setdiff(idx1,idx0), % new subjects only
        ...
        try, for nalt=1:numel(other{copyfromproject}.CONN_x.Setup.roifunctional), if isfield(other{copyfromproject}.CONN_x.Setup.roifunctional(nalt),'roiextract_functional')&&~isempty(other{copyfromproject}.CONN_x.Setup.roifunctional(nalt).roiextract_functional), CONN_x.Setup.roifunctional(nalt).roiextract_functional{nsub}=other{copyfromproject}.CONN_x.Setup.roifunctional(nalt).roiextract_functional{copyfromsubject}; end; end
        end
    ...
    try, for nalt=1:numel(CONN_x.Setup.roifunctional), if isfield(CONN_x.Setup.roifunctional(nalt),'roiextract_functional'), CONN_x.Setup.roifunctional(nalt).roiextract_functional={CONN_x.Setup.roifunctional(nalt).roiextract_functional{tempidx1}}; end; end
    end
...
if copyfiles && (value<value0||any(otheridx3(setdiff(idx1,idx0)))) && ~isempty(CONN_x.filename),
    ...
    for nother=1:length(other),
        ...
        filepathall{nother}=unique({filepath,filepathresults1,filepathresults2{:},filepathresults3{:},filepathresults4{:}});
        for n0=1:length(filepathall{nother}),
            files=dir(fullfile(filepathall{nother}{n0},['*',filenameskey,'*']));
            for n1=1:length(files),
                k=strfind(files(n1).name,filenameskey);
                if ~isempty(k),
                    k=k(1);

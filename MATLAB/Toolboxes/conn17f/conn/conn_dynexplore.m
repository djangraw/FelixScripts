...
boffset=[0 0 0 0];
conn_menu('frame2',boffset+[.15,.34,.82,.60],'');%'Component loadings');
%conn_menu('frame',boffset+[.44 .46 .16 .46],'');
poslist=boffset+[.17 .42 .30 .42];
...
%conn_dynexplore_play;
    ...
    function conn_dynexplore_mtnimag(varargin)
        ...
        if all(pos>0&pos<=numel(ROInames)),
            ...
            set(handleimag(end),'units','pixels','string',[{'Connectivity between:' ROInamesDisplay{idxorder(pos(2))} ['and ',ROInamesDisplay{idxorder(pos(1))}] ' ' 'Highest factor loadings for this connection:'} hf {' ' ['Highest factor loading for ' ROInamesDisplay{idxorder(pos(2))}]} hf2 {' ' ['Highest factor loading for ' ROInamesDisplay{idxorder(pos(1))}]} hf1]);
    ...
    function conn_dynexplore_mtnload(varargin)
        ...
        if all(pos>0&pos<=(numel(ROInames)+handleloaddoff)*handleloads1s2(1:2)),
            ...
            hf1=arrayfun(@(a,b)sprintf(' Circuit %d (%.3f)',a,b),Bhfc_order(1:min(size(B,1),3),idxorder(pos(1))),Bhfc_values(1:min(size(B,1),3),idxorder(pos(1))),'uni',0)';
            hf2=arrayfun(@(a,b)sprintf(' Circuit %d (%.3f)',a,b),Bhfc_order(1:min(size(B,1),3),idxorder(pos(2))),Bhfc_values(1:min(size(B,1),3),idxorder(pos(2))),'uni',0)';
            set(handleload(end),'units','pixels','string',[{ht 'Connectivity between:' ROInamesDisplay{idxorder(pos(2))} ['and ',ROInamesDisplay{idxorder(pos(1))}] ' ' 'Highest factor loadings for this connection:'} hf {' ' ['Highest factor loading for ' ROInamesDisplay{idxorder(pos(2))}]} hf2 {' ' ['Highest factor loading for ' ROInamesDisplay{idxorder(pos(1))}]} hf1]);
            %set(handleload(end),'units','pixels','string',[{ht 'Connectivity between:' ROInamesDisplay{idxorder(pos(2))} ['and ',ROInamesDisplay{idxorder(pos(1))}] ' ' 'Highest factor loadings for this connection:'} hf]);
    ...
    function conn_dynexplore_mtnplot(varargin)
        ...
        if nill2<.25^2,
            ...
            set(handleplot(end),'units','pixels','string',[{'Connectivity between:' ROInamesDisplay{pos(2)} ['and ',ROInamesDisplay{pos(1)}] ' ' 'Highest factor loadings for this connection:'} hf {' ' ['Highest factor loading for ' ROInamesDisplay{pos(2)}]} hf2 {' ' ['Highest factor loading for ' ROInamesDisplay{pos(1)}]} hf1]);
            %set(handleplot(end),'units','pixels','string',[{'Connectivity between:' ROInamesDisplay{pos(2)} ['and ' ROInamesDisplay{pos(1)}] ' ','Highest factor loadings:'} hf]);

...
METHOD=1;
if nargin<2||isempty(DisMin), DisMin=12; end
if nargin<3||isempty(SizeMin), SizeMin=.05; end
if iscell(filename)
    b=filename{1};
    mat=filename{2};
else
...
switch(METHOD)
    case 1
        ...
        nsources=0;
        for n1=1:numel(idx1)
            d=sqrt((sources(1:nsources,1)-xyz(n1,1)).^2+(sources(1:nsources,2)-xyz(n1,2)).^2+(sources(1:nsources,3)-xyz(n1,3)).^2);
            if all(d>DisMin)
                nsources=nsources+1;

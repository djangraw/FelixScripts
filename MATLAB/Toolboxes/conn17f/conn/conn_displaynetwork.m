...
%         else, nsources=varargin{1}; data.source=0; end
        ...
        data.namesreduced=regexprep(data.names2,{'^BA\.(\d+) \(([LR])\)\. .*','^\((-?\d+),(-?\d+),(-?\d+)\)$','^SLrois\.|^aal\.|^atlas\.','([^\(\)]+)\(.+)\s*$'},{'$1$2','($1 $2 $3)','','$1'});
        ...
        data.handles=[...
            ...
            uicontrol('style','popupmenu','units','norm','position',[.76,.78,.10,.04],'string',{'uncorrected','FDR corrected'},'foregroundcolor','k','backgroundcolor',color2,'callback',{@conn_displaynetwork,'thrtype'},'value',data.thrtype,'tooltipstring','Second-level analysis resultsfalse-positive control type'),...
            ...
            uicontrol('style','text','units','norm','position',[.53,.71,.45,.04],'string',sprintf('%-20s %6s  %6s  %4s  %8s  %8s','ROI','beta','T','dof','p-unc','p-FDR'),'foregroundcolor','b','backgroundcolor',color2,'fontname','monospaced','fontweight','bold','horizontalalignment','left'),...

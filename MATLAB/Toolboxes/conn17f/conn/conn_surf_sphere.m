function [ref,sphere2xyz,xyz2sphere]=conn_surf_sphere(resolution,xyz,issphere)
% sph=SURF_SPHERE(res) generates patch data (vertices/faces)
% for a unit sphere using a recursively-subdivided icosahedral grid.
%
% [sph,sph2xyz,xyz2sph]=SURF_SPHERE(res,xyz) returns additional
% coregistration indexes for secondary coordinates xyz 
% (Nx3 matrix of points in a unit sphere) such that:
%   xyz(xyz2sph,:) ~= sph.vertices
%   sph.vertices(sph2xyz,:) ~= xyz
%
% res vertices faces
% 1      12      20 (icosahedron)
% 2      42      80
...
for res=2:resolution
    ...
    if ~errorcheckxyz&&~isempty(xyz), % registers secondary surface
        ...
        % for each new-vertice, consider the four old-vertices surrounding it
        ...
        % for each xyz subset closest to one given old vertice, 
...
if resolution>8, disp('warning: Non-existing FreeSurfer ico reference. Files will not be compatible with freesurfer ico space');

...
if isdir(filename)
    filename=fullfile(filename,'aseg.mgz');
end
...
if justchecking
    ...
    elseif ok2, 
        fprintf('Note: Files %s already exist. Skipping conversion\n',sprintf('%s ',filenames{:}));
    else
        a=conn_getinfo(filename);
        b=spm_read_vols(a);
        idxvoxels=b>0;
        ...
        for nreg=1:3,
            ...
            disp(['Saved file ',V.fname,' (',num2str(nnz(b_reg{nreg})),' voxels)']);
        end
    end
end

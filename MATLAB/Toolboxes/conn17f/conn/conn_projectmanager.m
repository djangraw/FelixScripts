    ...
    case 'updateproject' % merges any extended projects (run after loading base project)
        ...
        if CONN_x.pobj.holdsdata
            ...
            if ~isempty(allfiles),
                ...
                tag=unique([tag{:}]);
                for n=1:numel(tag)
                    pathname=fullfile(conn_prepend('',CONN_x.filename,'.qlog'),tag{n});
                    if exist(pathname,'dir')&&conn_existfile(fullfile(pathname,'info.mat'))
                        ...
                        info=conn_jobmanager('statusjob',info);
                        ...
                        if ~all(ismember(info.tagmsg,validlabels)),
                            if dogui&&(isequal(CONN_x.gui,1)||(isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'display')&&CONN_x.gui.display))
                                ...
                                fprintf('Warning: pending jobs in %s not finished yet. Until then, any modifications to this project may be overwritten once the pending jobs finish and they are merged back into this project\n',pathname);
                                return;
                            end
                        end
                    end
            ...
            if ~isempty(allfiles)
                disp(allfiles);
                fprintf('Merging finished jobs. Please wait...');
                filename=CONN_x.filename;
                pobj=CONN_x.pobj;
                temp=load(deblank(allfiles(1,:)),'CONN_x','-mat');
                if temp.CONN_x.pobj.holdsdata
                    conn_merge(allfiles);
                else
                    CONN_x=temp.CONN_x;
            ...
            if ~isempty(allfiles)
                disp(allfiles);
                fprintf('Performing delayed processing steps. Please wait...');
                psteps={};
    ...
    case 'cleanproject'  % removes any delayed-writing files
        if CONN_x.pobj.holdsdata
            for n=1:numel(CONN_x.pobj.importedfiles)
                ...
                else     [ok,nill]=system(['rm ''',deblank(CONN_x.pobj.importedfiles{n}),'''']);
                end
            end

...
if any(~ok2), disp('WARNING: some of the selected Set-2 ROIs have not been defined as ''sources'' in the CONN toolbox first-level analyses. Within-network computations can only be computed for ROIs defined as ''sources''. Within-Set-2 computations will now skip these ROIs'); end
...
fprintf('Set-2 Within-network test (average effect between Set-2 ROIs):\n');
[h2,F2,p2,dof2,statsname2]=conn_glm(X,Z_within2,csubjecteffects,cconditions);
fprintf(' %s(%s)=%.2f  (p=%.6f)\n',statsname2,sprintf('%d ',dof2),F2,p2);
fprintf('Between-networks test (average effect between Set-1 & Set-2 ROIs):\n');

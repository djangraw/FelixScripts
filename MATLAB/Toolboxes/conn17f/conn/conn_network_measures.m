...
measure_names={'GlobalEfficiency','LocalEfficiency','BetweennessCentrality','Cost','AveragePathLength','ClusteringCoefficient','Degree'};
if ~nargin
    measure_values={}; 
    return
else
    ...
    E_global=nan(N,1);E_local=E_global;PathLength=E_global;ClCoef=E_global;K=nan(N,1);Degree=K;
    for n=1:N,
        d=find(C(n,:));
        nd=numel(d);
        Degree(n)=nd;
        connected=~isinf(D(n,:))&D(n,:)>0;
        if nnz(connected)>0, PathLength(n)=mean(D(n,connected)); end
        if nd>1,
    ...
    measure_values={E_global,E_local,W,K,PathLength,ClCoef,Degree};
end

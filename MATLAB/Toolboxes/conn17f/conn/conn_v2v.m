...
switch(lower(option))
    case {'measures','empty','measurenames'}
        measures=struct(...
            ...
            'alt_names',{{{'group PCA','PCA'},{'group ICA','ICA'},{'group MVPA','MVPA'},{'IntrinsicConnectivityContrast','ICC'},{'IntegratedLocalCorrelation','ILC','LCOR'},{'GC','GCOR'},{'RadialCorrelationContrast','RCC'},{'RadialSimilarityContrast','RSC'},{'AmplitudOfLowFrequencyFluctuations'},{'FractionalAmplitudOfLowFrequencyFluctuations','FALFF'}}},...
            'measuretype',{{4,3,2,1,1,5,1,1,6,7}},...
            'localsupport',{{nan,nan,nan,0,8,inf,8,0,nan,nan}},...
            'global',{{nan,nan,nan,1,0,0,0,1,nan,nan}},...
            'deriv',{{nan,nan,nan,0,0,0,1,1,nan,nan}},...
            'norm',{{nan,nan,0,1,1,1,1,1,1,1}},...
            'mask',{{{},{},{},{},{},{},{},{},{},{},{},{}}},...
            'filename',{{'','','','','','','','','',''}},...
            'dimensions_out',{{20,20,4,1,1,1,3,3,1,1}},...
    ...
    case 'compute_end'
        ...
        if ~isfield(options,'norm')||options.norm
            if ~iscell(options.m)
                ...
                [nill,idxsort]=sort(options.m(idx));
                idxsort(idxsort)=spm_invNcdf((1:numel(idxsort))/(numel(idxsort)+1));
                options.m(idx)=idxsort;
                %options.m(idx)=(options.m(idx)-mean(options.m(idx)))/std(options.m(idx));
            else
                for n1=1:numel(options.m)
                    idx=find(~isnan(options.m{n1})&options.m{n1}~=0);
                    [nill,idxsort]=sort(options.m{n1}(idx));
                    idxsort(idxsort)=spm_invNcdf((1:numel(idxsort))/(numel(idxsort)+1));
                    options.m{n1}(idx)=idxsort;
...
%         end
        if ~isfield(options,'norm')||options.norm
            if ~iscell(varargout{1})
                idx=find(~isnan(varargout{1})&varargout{1}~=0);
                [nill,idxsort]=sort(varargout{1}(idx));
                idxsort(idxsort)=spm_invNcdf((1:numel(idxsort))/(numel(idxsort)+1));
                varargout{1}(idx)=idxsort;
                %varargout{1}(idx)=(varargout{1}(idx)-mean(varargout{1}(idx)))/std(varargout{1}(idx));
            else
                for n1=1:numel(varargout{1})
                    idx=find(~isnan(varargout{1}{n1})&varargout{1}{n1}~=0);
                    [nill,idxsort]=sort(varargout{1}{n1}(idx));
                    idxsort(idxsort)=spm_invNcdf((1:numel(idxsort))/(numel(idxsort)+1));
                    varargout{1}{n1}(idx)=idxsort;

...
if ~nargin||isempty(varargin), 
    ...
    try
        answ=conn_questdlg({'Preparing to download and analyze the publicly available NYU test-retest dataset (http://www.nitrc.org/projects/nyu_trt/)','This procedure will create a new CONN project with the fully analyzed NYU test-retest dataset','The full dataset requires approximately 200Gb of hard-drive space and 30 hours to finish. You may download/process the full dataset or only a portion of it','Do you want to proceed?'},'Sample data download','Full dataset','Partial dataset','Already downloaded dataset','Cancel','Full dataset');
        ...
        if isequal(answ,'Partial dataset')
            ...
            data=data(s);
        elseif isequal(answ,'Already downloaded dataset')
            ...
            str='Select target folder to store the new CONN project and data';
            disp(str);
            cwd=uigetdir(pwd,str);
        end
        if isequal(cwd,0), return; end

...
if nargin<1,
    ...
    if ~isfield(CONN_x.Setup,'roifunctional'), 
    ...
    if ~isfield(CONN_x,'vvAnalyses'), CONN_x.vvAnalyses=struct('name','','measurenames',{{}},'variables', conn_v2v('measures'),'regressors', conn_v2v('measures'),'mask',[]); end
    if ~isfield(CONN_x,'vvAnalysis'), CONN_x.vvAnalysis=1; end
    for ianalysis=1:numel(CONN_x.vvAnalyses)
        if ~isfield(CONN_x.vvAnalyses(ianalysis),'name'), CONN_x.vvAnalyses(ianalysis).name=''; end
    ...
    if ~iscell(conn_x.Setup.cwthreshold)
        ithr=[3,1,2];
        if isfield(conn_x.Setup,'cwthreshold')&&numel(conn_x.Setup.cwthreshold)>0, THRall=conn_x.Setup.cwthreshold(1:2:end);
        else THRall=.5;  % default threshold for white/CSF probability maps before erosion
        end
        if isfield(conn_x.Setup,'cwthreshold')&&numel(conn_x.Setup.cwthreshold)>1, ERODEall=conn_x.Setup.cwthreshold(2:2:end);
        else ERODEall=1; % default erosing level for white/csf masks (voxels) (set to 0 for no erosion)
        end
        for nroi=1:3 % fills-out defaults in a back-compatible manner if these values are not defined
            if nroi==1,
            ...
            conn_x.Setup.cwthreshold(2*ithr(nroi)-1)=THR;
            conn_x.Setup.cwthreshold(2*ithr(nroi))=ERODE;
        end
        conn_x.Setup.cwthreshold=reshape(conn_x.Setup.cwthreshold,2,3);
        conn_x.Setup.cwthreshold=num2cell([conn_x.Setup.cwthreshold(:,ithr);ones(1,3)],1);
    end
    if ~isfield(conn_x.Setup.erosion,'binary_threshold_type'), conn_x.Setup.erosion.binary_threshold_type=[1 1 1]; end
    if ~isfield(conn_x.Setup,'acquisitiontype'), 
        if isfield(conn_x.Setup,'sparseacquisition'), conn_x.Setup.acquisitiontype=1+(conn_x.Setup.sparseacquisition>0);

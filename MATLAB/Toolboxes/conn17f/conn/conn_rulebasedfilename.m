...
default_rule={{1,'^s',''},{1,'^sw',''},{1,'.*','s$0'},{1,'.*','sw$0'},{2,'func_smoothed','func_normalized'}}; % same as functional without leading 's' character (SPM convention for unsmoothed volume filenames)
...
if nargin<2
    ...
    switch(filename)
        case 'functional'
            ...
            filename={};
            if isequal(ok,true)
                ht=conn_msgbox('Changing file references. Please wait...','');
        ...
        case 'structural'
            ...
            [rule,ok]=conn_rulebasedfilename(filename,0,[],CONN_x.Setup.structural,'structural',true);
            filename={};
            if isequal(ok,true)
                ht=conn_msgbox('Changing file references. Please wait...','');
                for nsub=1:CONN_x.Setup.nsubjects
...
switch option
    case 0, % gui edit rules
        ...
        uicontrol('units','norm','position',[.05,.85,.25,.07],'style','text','string','Rule:','backgroundcolor',.9*[1 1 1],'horizontalalignment','left','fontweight','bold','fontsize',8+CONN_gui.font_offset);
        ...
        uicontrol('units','norm','position',[.05,.50,.9,.05],'style','text','string','Original filename:','backgroundcolor','w','horizontalalignment','left','fontweight','bold','fontsize',8+CONN_gui.font_offset);
        ...
        uicontrol('units','norm','position',[.05,.30,.9,.05],'style','text','string','New filename:','backgroundcolor','w','horizontalalignment','left','fontweight','bold','fontsize',8+CONN_gui.font_offset);

...
uicontrol(thfig,'style','pushbutton','string','Ok','units','norm','position',[.1,.025,.38,.07],'callback',@conn_convertl12l2covariate_ok,'fontsize',8+CONN_gui.font_offset,'tooltipstring','Compute summary measures and import them as new 2nd-level covariates');
uicontrol(thfig,'style','pushbutton','string','Cancel','units','norm','position',[.51,.025,.38,.07],'callback','delete(gcbf)','fontsize',8+CONN_gui.font_offset);
conn_convertl12l2covariate_update;

    function conn_convertl12l2covariate_ok(varargin)
        ok=ishandle(thfig);
        if ok,
            ...
            ht=conn_waitbar(0,'Loading covariate/condition info. Please wait...');
    ...
    function conn_convertl12l2covariate_update(varargin)
        ...
        if order 
            str=['$$',measures{n2,3}(measures{n3,3}(str,'k',K1,K2,K3),'n',N1,N2,N3),'$$'];
            set(hs1b,'position',[.1,.78,.8,.05]);
            set(ht1b,'position',[.1,.73,.8,.05]);
            set(hs1a,'position',[.1,.66,.8,.05]);
            set(ht1a,'position',[.1,.61,.8,.05]);
        else
            str=['$$',measures{n3,3}(measures{n2,3}(str,'n',N1,N2,N3),'k',K1,K2,K3),'$$'];

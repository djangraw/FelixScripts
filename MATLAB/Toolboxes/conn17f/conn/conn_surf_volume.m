...
elseif ischar(filename), VOL=spm_vol(filename); fname=filename;
...
if ~isempty(fname)&&~any(rem(DATA(:),1))
    try
        maxdata=max(DATA(:));
        trefnames=regexp(fileread(conn_prepend('',fname,'.txt')),'\n*','split');
        trefnames=trefnames(cellfun('length',trefnames)>0);
        if numel(trefnames)==maxdata, refnames=trefnames; reftype=1; if DODISP, THR=1; else THR=1:maxdata; end
        elseif numel(trefnames)==size(DATA,4), refnames=trefnames; reftype=2; if DODISP, THR=1; else THR=1:size(DATA,4); end
        end
    end
end
DOLOGHIST=false;
...
if ishandle(hfig)
    ...
    function h=conn_surf_volume_update(varargin)
        ...
        else HANDLES(1)=uicontrol('style','listbox','units','norm','position',[.05,.30,.4,.40],'string',refnames,'max',2,'value',THR,'tooltipstring','<HTML>Select ROIs</HTML>','callback',@conn_surf_volume_update,'parent',hfig); 

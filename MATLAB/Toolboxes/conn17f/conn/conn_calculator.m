...
%         icovariates1all=icovariates1all(i);
%     end
    icovariates1=icovariates1all(cellfun(@(x)isempty(regexp(x,'^Dynamic |^_')),CONN_x.Setup.l2covariates.names(icovariates1all)));
    ...
    mnames=regexprep(mnames,'^Dynamic\s*|^_','');
...
if fulltype, 
    ...
    conn_menu('pushbutton',[fs0 0 0]+[fs fs].*[.90,1,.10,.05],'','Help','','conn(''gui_help'',''help'',''conn_calculator.m'');'); 
    ...
    %thfig=figure('units','norm','position',[.15,.3,.8,.5],'name','conn second-level calculator','numbertitle','off','menubar','none','color',[1 1 1]);
    conn_menu('frame',[fs0 fs+[0 .06]],'');%Second-level design');
    ht1=conn_menu('listbox',[fs0 0 0]+[fs fs].*[.02,.25,.30,.65],'Subject effects',conn_strexpand(CONN_x.Setup.l2covariates.names(icovariates1),CONN_x.Setup.l2covariates.descrip(icovariates1)),'Select subject effect(s) characterizing second-level analysis model / independent variables',@(varargin)conn_calculator_update);
    ht3=conn_menu('edit',[fs0 0 0]+[fs fs].*[.02,.10,.30,.05],'Between-subjects contrast','1','<HTML>Define desired contrast across selected subject-effects<br/> - enter contrast vector/matrix with as many elements/columns as subject-effects selected <br/> - use the list below to see a list of standard contrasts for the selected subject-effects <br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>',@(varargin)conn_calculator_update);
    ht2c=conn_menu('listbox',[fs0 0 0]+[fs fs].*[.34,.25,.30,.65],'Conditions',ucnames,'Select condition(s)',@(varargin)conn_calculator_update);
    ht4c=conn_menu('edit',[fs0 0 0]+[fs fs].*[.34,.10,.30,.05],'Between-conditions contrast','1','<HTML>Define desired contrast across selected conditions <br/> - enter contrast vector/matrix with as many elements/columns as selected conditions <br/> - use the list below to see a list of standard contrasts for the selected condition<br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>',@(varargin)conn_calculator_update);
    ht2=conn_menu('listbox',[fs0 0 0]+[fs fs].*[.66,.25,.30,.65],'Measures',umnames,'<HTML>Select outcome measure(s) <br/> - measures represent properties of timecourses for each subject/condition</HTML>',@(varargin)conn_calculator_update);
    ht4=conn_menu('edit',[fs0 0 0]+[fs fs].*[.66,.10,.30,.05],'Between-measures contrast','1','<HTML>Define desired contrast across selected measures <br/> - enter contrast vector/matrix with as many elements/columns as measures selected <br/> - use the list below to see a list of standard contrasts for the selected measures<br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>',@(varargin)conn_calculator_update);
    hc1b=uicontextmenu;
    ...
    ht5=conn_menu('textedit2',[fs0 0 0]-[0 .15 0 0]+[fs fs].*[.02,.0,.96,.10],'Analysis results','','Effect size and statistics for selected second-level analysis');
end
handleplot=[];
% try
% if domacbugfix,

function [ALLEEG, v, Azloo, time] = run_logisticregression_jittered_EM_wrapper_RT_v3p1(subject,weightprior,cvmode,jitterrange,sigmamultiplier,windowOffset,resplock,lambda,suffix)

% Runs jittered logistic regression (EM version) on a given subject using 
% pre-defined paramters.
%
% [ALLEEG, v, Azloo, time] = run_logisticregression_jittered_EM_wrapper_RT_v3p1
%                  (subject,weightprior,cvmode,jitterrange,sigmamultiplier,
%                   windowOffset,resplock,lambda,suffix)
%
% INPUTS:
...
% Updated ~11/12 by DJ - v3p0, v3p1
% Updated 12/18/12 by DJ - added suffix input
% TO DO: FIX OUTPUTS!
...
elseif isequal(scope_settings.jitter_fn,@computeJitterPrior_templatematch) || isequal(scope_settings.jitter_fn,@computeJitterPrior_templatematch_exgauss)
    scope_settings.jitterparams.smoothwidth = 50; %50; 
...
%     iChans = find(ismember({ALLEEG(1).chanlocs.labels},{'Oz' 'Cz' 'Fz' 'P3' 'P4'}));
    ...
    scope_settings.jitterparams.useMax = true;
...
run_logisticregression_jittered_EM_gaussian_v3p1(outDirName,...
											ALLEEG,...
											setlist,...
											chansubset,...
											scope_settings,...

...
close all
clear all
addpath('/data/SFIMJGC/Apps/MatlabAddOns/');
addpath('/data/SFIMJGC/Apps/MatlabAddOns/NCestimation/');
addpath('/data/SFIMJGC/PRJ_CognitiveStateDetection01/Scripts/');
addpath('/data/SFIMJGC/PRJ_CognitiveStateDetection01/Scripts/CSD_Toolbox/');
addpath('/data/SFIMJGC/Apps/MatlabAddOns/Clustering/');
% Subject & Run Information
  Subject = char(strcat('SBJ',num2str($SBJ,'%02d'))); %'SBJ06';%char(strcat('SBJ',num2str($SBJ,'%02d')));
  NumAcq  = 1017;
  TR      = 1.5;
% Data Location
  PrjDir    = '/data/SFIMJGC/PRJ_CognitiveStateDetection01';
  TaskID    = 'CTask001';
  DirPrefix = 'D02';
% Atlas Info
  NumROIsAtlas = $NROI;
...
%  ---------------------
if (isSorted == 0 )
   filename=char(strcat(fileDir,'/',Subject,'_',DimRed_Methods(DimRed_Method),'_NROI',num2str(NumROIsAtlas,'%04d'),'_WL',num2str(floor(winLength_inTR),'%04d'),'_WS',num2str(winStep_inTR,'%04d'),'_VK',num2str(VarToKeep,'%06.02f'),'.UNSORTED')); 
else

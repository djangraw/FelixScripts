function [matchStrength_T,winInfo,matchStrength_negT] = GetRosenbergScoreForCogStateData(subjNum)

% [matchStrength,winInfo] = GetRosenbergScoreForCogStateData(subjNum)
%
...
fprintf('[DONE]\n');
FcTemplate_negT = attnNets.pos_overlap; % Rosenberg high-attention network
fprintf('[DONE]\n');

%% ADJUST TO MATCH
nT = size(timecourses,1);
nROI = size(FcTemplate_T,2);
% Adjust to match
...
for iWin = 1:nWin
    ...
    % Calculate match
    matchStrength_T(iWin) = GetFcTemplateMatch(timecourses(~isZeroRow & isInWin,~isZeroCol)',FcTemplate_T(~isZeroCol,~isZeroCol),sum(~isZeroRow & isInWin),normFC,matchMethod);
    fprintf('[DONE]\n');

function [matchStrength,winInfo] = GetRosenbergScoreForCogStateData(subjNum)
...
for iWin = 1:nWin
    ...
    % Calculate match
    matchStrength(iWin) = GetFcTemplateMatch(timecourses(~isZeroRow & isInWin,~isZeroCol)',FcTemplate(~isZeroCol,~isZeroCol),sum(~isZeroRow & isInWin),normFC,matchMethod);
    fprintf('[DONE]\n');
end

...
# Updated 4/22/16 by DJ - temporary switch to NonRandomAtlas version
###########################################################################
...
outFile=${SBJ}_${RUN}.${AtlasID}_${iRand}_10VoxelMin.lowSigma.WL${WL}.1D
if [ -s $outFile ]; then
    echo "=== FILE $iRand EXISTS... SKIPPING ==="
    continue
else
    echo "=== FILE $iRand IS EMPTY OR MISSING... RUNNING! ==="
fi

...
tsFilename = sprintf('%s/%s_WholeBrainSvd.mat',fileDir,Subject); % end-1 is to avoid double .
foo = load(tsFilename);
origTS = foo.S*foo.V';
...
pctVar = cumsum(diag(foo.S).^2)/sum(diag(foo.S).^2)*100;
nPCs = find(pctVar<VarToKeep,1,'last');
PCARes = struct('numPCAs',nPCs,'pcaTS',origTS(1:nPCs,:)');
...
%  ---------------------
if (isSorted == 0 )
   filename=char(strcat(fileDir,'/',Subject,'_',DimRed_Methods(DimRed_Method),'_NROI',num2str(NumROIsAtlas,'%04d'),'_WL',num2str(floor(winLength_inTR),'%04d'),'_WS',num2str(winStep_inTR,'%04d'),'_VK',num2str(VarToKeep,'%06.02f'),'_WholeBrainSvd.UNSORTED')); 
else
   filename=char(strcat(fileDir,'/',Subject,'_',DimRed_Methods(DimRed_Method),'_NROI',num2str(NumROIsAtlas,'%04d'),'_WL',num2str(floor(winLength_inTR),'%04d'),'_WS',num2str(winStep_inTR,'%04d'),'_VK',num2str(VarToKeep,'%06.02f'),'_WholeBrainSvd.SORTED',num2str(sortMetric,'%02d')));
end
switch ROIConnMetric
...
clear err errMsg win PrjDir DirPrefix winLength_inTR winStep_inTR wallTime resFig plotDist plotMusic ans foo i
save(filename)

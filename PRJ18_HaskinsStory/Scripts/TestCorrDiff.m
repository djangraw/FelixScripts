function [p, z, za, zb] = TestCorrDiff(ra, rb, na, nb)
%[p, z, za, zb] = TestCorrDiff(ra, rb, na, nb)
%
...
%
% corr_rtest created by Ryosuke F Takeuchi 2017/02/02 - 
% Downloaded 4/9/19 from MathWorks File Exchange
% Updated 4/9/19 by DJ - renamed and commented.

    % convert r values to z scores
	za  = r2z(ra);
	zb  = r2z(rb);
	% find z score of difference
    szab = sqrt(1/(na-3) + 1/(nb-3));
	z = abs(za-zb)/szab;
    % convert z score to p values
	p(1) = 1-normcdf(z, 0, 1); % one-tailed
	p(2) = 2*p(1);	 % two-tailed
end

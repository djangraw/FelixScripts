...
def var_comp_rst(variables,num_report):
	...
	for k in range(len(variables)):
		for i in range(len(variables[k])):
			...
			while len(N_) < len(str(num_report)):
				N_ = '0' + N_
			var_comp = str(int(variables[k][i][11][:variables[k][i][11].index('.')]))
			while len(var_comp) < len(str(len(glob.glob('%s/Report_Figures/Axial_Component_*' % os.path.dirname(os.path.expanduser(pattern[k][i])))))):
				var_comp = '0' + var_comp
...
if __name__=='__main__':
	...
	opt.add_argument('-var_component', dest = 'var_comp', help = 'If specified, will show highest variance accepted componet from each report', action = 'store_true',default = False)
	...
	for k in range(len(variables)):
		for i in range(len(variables[k])):
			...
			if args.num_comp != False:
				subprocess.call('cp  %s/Report_Figures/Axial_Component_%s.png ./Report_Figures/Num_Component_%s_%s.png' % 
			...
			if args.var_comp != False:
				...
				subprocess.call('cp  %s/Report_Figures/Axial_Component_%s.png ./Report_Figures/Var_Component_%s_%s.png' % (os.path.dirname(os.path.expanduser(pattern[k][i])),
	...
	subprocess.call('mv make.bat sphinx_files', shell = True)

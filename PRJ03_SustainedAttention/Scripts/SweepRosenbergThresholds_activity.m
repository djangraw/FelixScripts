function [maskSizePos,maskSizeNeg,Rsq,p,r,p_spearman,r_spearman] = SweepRosenbergThresholds_activity(cp,cr,roiActivity,fracCorrect,thresholds,doPlot)

% [maskSizePos,maskSizeNeg,Rsq,p,r,p_spearman,r_spearman] = SweepRosenbergThresholds_activity(cp,cr,roiActivity,fracCorrect,thresholds,doPlot)
%
% INPUTS:
% -cp and cr are mxn matrices (m = # ROIs, n = # subjects) of the
% p-values and r-values for the correlation of activity with behavior
% (fracCorrect). 
% -roiActivity is an mxn matrix of activity matrices for each subject.
...
% Updated 3/21/17 by DJ - made into _activity version
...
for i=1:nThresh
    ...
    for j=1:nSubj
        ...
        % Get roiActivity match scores
        matchPos(i,j) = nanmean(roiActivity(:,j).*isPos);
        matchNeg(i,j) = nanmean(roiActivity(:,j).*isNeg);
        matchBoth(i,j) = nanmean(roiActivity(:,j).*(double(isPos)-double(isNeg)));
    end
    
    if ~any(isnan(matchPos(i,:)))
        % get overlap mask sizes
        isPos_all = all(isPos_subj,2);
        isNeg_all = all(isNeg_subj,2);
        ...
        r1 = corr(fracCorrect(:),matchPos(i,:)');
        ...
        r2 = corr(fracCorrect(:),matchNeg(i,:)');
        ...
        r4 = corr(fracCorrect(:),matchBoth(i,:)');

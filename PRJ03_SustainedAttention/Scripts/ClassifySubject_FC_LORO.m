function [AzCv, fwdModelCv, statsCv, newParams, AzCvPerm, fwdModelCvPerm, statsCvPerm, yCv_allTrials] = ClassifySubject_FC_LORO(subject,allParams)

% [AzCv, fwdModelCv, statsCv, newParams, AzCvPerm, fwdModelCvPerm, statsCvPerm, yCv_allTrials] = ClassifySubject_FC_LORO(subject,allParams)
%
% Classify a single subject's data - with LORO CV - using Functional
...
% Updated 8/26/16 by DJ - allow custom atlasType and switched to LORO CV.
...
for j=1:nRuns % for each run
    ...
    nTrainInRun(j,k) = sum(isTrain & fcRuns==j);
    nTrainInCond(j,k) = nan;%sum(isTrain & fcRuns==fcRuns(isTest) & fcTruth==fcTruth(isTest));
...
%         yTest = statsCv(j,k).wts(1:end-1)*fcFeats(:,iTest) + statsCv(j,k).wts(end);
    ...
    yTest = statsCv(j,k).wts(1:end-1)'*fcFeats(:,isTest) + statsCv(j,k).wts(end);
    yCv(isTest,k) = yTest;
...
%                 nTrainInCond(j,k) = sum(isTrain & fcRuns==fcRuns(iTest) & fcTruth==fcTruth(iTest));
    ...
    %         yTest = statsCvPerm(j,k,i).wts(1:end-1)*fcFeats(:,iTest) + statsCvPerm(j,k,i).wts(end);
            ...
            yTest = statsCvPerm(j,k,i).wts(1:end-1)'*fcFeats(:,isTest) + statsCvPerm(j,k,i).wts(end);

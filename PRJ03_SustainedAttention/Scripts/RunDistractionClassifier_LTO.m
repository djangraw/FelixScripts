function [y_true,y_perm] = RunDistractionClassifier_LTO(subject,label1,label0,useEcho2,doPerms,fracTcVarToKeep,fracFcVarToKeep,doFMs,doPlots)

% [y_true,y_perm] = RunDistractionClassifier(subject,label1,label0,useEcho2,doPerms,fracTcVarToKeep,fracFcVarToKeep,doFMs,doPlots)
%
...
% Updated 3/1/16 by DJ - added doPlots input
...
%     cd /Users/jangrawdc/Documents/PRJ03_SustainedAttention/Pilots/fMRI/S06_2015-12-17
    ...
    if useEcho2, load('SBJ06_FC_MultiEcho_2015-12-17_Craddock_ECHO2.mat');
    else load('SBJ06_FC_MultiEcho_2015-12-17_Craddock.mat'); 
    end   
    question = questions;
...
[~,AzLoo_bold0,LRstats_bold,AzLto_bold] = RunSingleLR(permute(normfeats(isBoldFeat,isOkTrial),[1 3 2]),truth(isOkTrial),params);
if sum(isFcFeat)>0
    [~,AzLoo_fc0,LRstats_fc,AzLto_fc] = RunSingleLR(permute(normfeats(isFcFeat,isOkTrial),[1 3 2]),truth(isOkTrial),params);
else
    fprintf('No FC features... classifier can''t run!\n')
    AzLoo_fc0 = 0;
end
[~,AzLoo_eye0,LRstats_eye,AzLto_eye] = RunSingleLR(permute(normfeats(isEyeFeat,isOkTrial),[1 3 2]),truth(isOkTrial),params);
[~,AzLoo_fmri0,LRstats_fmri,AzLto_fmri] = RunSingleLR(permute(normfeats(isBoldFeat | isFcFeat,isOkTrial),[1 3 2]),truth(isOkTrial),params);
[~,AzLoo_all0,LRstats_all,AzLto_all] = RunSingleLR(permute(normfeats(:,isOkTrial),[1 3 2]),truth(isOkTrial),params);

% Print results
% fprintf('LOO Az: BOLD = %.3f, FC = %.3f, EYE = %.3f, FMRI = %.3f, ALL = %.3f\n',AzLoo_bold,AzLoo_fc,AzLoo_eye,AzLoo_fmri,AzLoo_all);
fprintf('LTO Az: BOLD = %.3f, FC = %.3f, EYE = %.3f, FMRI = %.3f, ALL = %.3f\n',AzLto_bold,AzLto_fc,AzLto_eye,AzLto_fmri,AzLto_all);

%% Get y for all trials
y_bold = mean(LRstats_bold.wtsLto,2)'*[normfeats(isBoldFeat,:); ones(1,size(normfeats,2))];
y_bold(isOkTrial) = LRstats_bold.yLto;
if sum(isFcFeat)>0
    y_fc = mean(LRstats_fc.wtsLto,2)'*[normfeats(isFcFeat,:); ones(1,size(normfeats,2))];
    y_fc(isOkTrial) = LRstats_fc.yLto;
else
    y_fc = zeros(size(y_bold));
end
y_eye = mean(LRstats_eye.wtsLto,2)'*[normfeats(isEyeFeat,:); ones(1,size(normfeats,2))];
y_eye(isOkTrial) = LRstats_eye.yLto;
y_fmri = mean(LRstats_fmri.wtsLto,2)'*[normfeats(isBoldFeat | isFcFeat,:); ones(1,size(normfeats,2))];
y_fmri(isOkTrial) = LRstats_fmri.yLto;
y_all = mean(LRstats_all.wtsLto,2)'*[normfeats; ones(1,size(normfeats,2))];
y_all(isOkTrial) = LRstats_all.yLto;
...
outPlotAzs = [AzLto_bold, AzLto_fc, AzLto_eye, AzLto_fmri, AzLto_all]; 
...
% Get Azs for each pair of labels
...
if doPlots
    ...
    for j=1:size(outPlot,1)
        ...
        axis square;
        colorbar
        try % sometimes gives error on Helix/Felix
...
Az_true = outPlotAzs;
Az_LTO = [AzLto_bold, AzLto_fc, AzLto_eye, AzLto_fmri, AzLto_all]; 
y_true = outPlot;
AzPairs_true = AzPairs;
LRstats_true = [LRstats_bold, LRstats_fc, LRstats_eye, LRstats_fmri, LRstats_all]; 

%% Run permutations
nPerms = 100;
if doPerms    

    [Az_perm, y_perm, AzPairs_perm,Az_perm_LTO,LRstats_perm] = deal(cell(1,nPerms));
    for iPerm=1:nPerms
        fprintf('Perm %d/%d...\n',iPerm,nPerms);
        % permute truth
        ...
        % get results
        [~,AzLoo_bold0,LRstats_bold,AzLto_bold] = RunSingleLR(permute(normfeats(isBoldFeat,isOkTrial),[1 3 2]),permtruth,params);
        [~,AzLoo_fc0,LRstats_fc,AzLto_fc] = RunSingleLR(permute(normfeats(isFcFeat,isOkTrial),[1 3 2]),permtruth,params);
        [~,AzLoo_eye0,LRstats_eye,AzLto_eye] = RunSingleLR(permute(normfeats(isEyeFeat,isOkTrial),[1 3 2]),permtruth,params);
        [~,AzLoo_fmri0,LRstats_fmri,AzLto_fmri] = RunSingleLR(permute(normfeats(isBoldFeat | isFcFeat,isOkTrial),[1 3 2]),permtruth,params);
        [~,AzLoo_all0,LRstats_all,AzLto_all] = RunSingleLR(permute(normfeats(:,isOkTrial),[1 3 2]),permtruth,params);
        ...
        outPlot = [y_bold; y_fc; y_eye; y_fmri; y_all];
        outPlotAzs = [AzLto_bold, AzLto_fc, AzLto_eye, AzLto_fmri, AzLto_all]; 
        outPlotAzs_LTO = [AzLto_bold, AzLto_fc, AzLto_eye, AzLto_fmri, AzLto_all];
        ...
        Az_perm_LTO{iPerm} = outPlotAzs_LTO;
        y_perm{iPerm} = outPlot;
        AzPairs_perm{iPerm} = AzPairs;
        LRstats_perm{iPerm} = [LRstats_bold, LRstats_fc, LRstats_eye, LRstats_fmri, LRstats_all]; 

    end
    fprintf('Done!\n')
    %% SAVE!
    save(sprintf('SBJ%02d_MultimodalClassifier_%s-%s_%s',subject,label1,label0,datestr(now,'YYYY-mm-DD')),'Az_true','y_true','AzPairs_true','Az_perm','Az_perm_LTO','y_perm','AzPairs_perm','subject','fcWinLength','TR','nFirstRemoved','fracTcVarToKeep','fracFcVarToKeep','labels','labelNames','label0','label1','LRstats_true','LRstats_perm');
    ...
    % SAVE!
    save(sprintf('SBJ%02d_MultimodalClassifier_%s-%s_%s',subject,label1,label0,datestr(now,'YYYY-mm-DD')),'Az_true','y_true','AzPairs_true','subject','fcWinLength','TR','nFirstRemoved','fracTcVarToKeep','fracFcVarToKeep','label0','label1','Az_LTO','LRstats_true');    

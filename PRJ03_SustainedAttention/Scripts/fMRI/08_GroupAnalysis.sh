...
# Updated 1/12/17 by DJ - remove normalization (it's already done now)
...
do
    ...
    3dmerge -1blur_fwhm 4.0 -doall -overwrite -prefix rm.coef_stimOnly.${subj}.blur_fwhm4p0 stats_stimOnly.${subj}+tlrc'[1..$(3)]' # only _Coef bricks
        # NORMALIZE
        # 3dTstat -mean -overwrite -prefix rm.mean_all_runs.${subj}
        # all_runs.${subj}+tlrc. 3dcalc -a
        # rm.coef_stimOnly.${subj}.blur_fwhm4p0+tlrc -b
        # rm.mean_all_runs.${subj}+tlrc \
        #        -c mask_anat.${subj}+tlrc                            \
        #        -expr 'c * min(200, a/b*100)'       \
        #        -overwrite \
        #        -prefix coef_stimOnly.${subj}.blur_fwhm4p0.scale
        # MASK BLURRED DATA
        3dcalc -a rm.coef_stimOnly.${subj}.blur_fwhm4p0+tlrc \
    ...
    rm rm.coef_stimOnly.${subj}.blur_fwhm4p0+tlrc*
    # RECORD OUTPUT NAME

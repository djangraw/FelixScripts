...
if ($subj == SBJ36 || $subj == SBJ31) then
    ...
    @auto_tlrc -base TT_N27+tlrc -input ${subj}_Anat_bc_ns_al_keep+orig -no_ss -pad_base 60
else
    @auto_tlrc -base TT_N27+tlrc -input ${subj}_Anat_bc_ns_al_keep+orig -no_ss
endif
...
3dresample -master full_mask.$subj+tlrc -prefix ./rm.resam.group             \
           -input /home/jangrawdc/Apps/abin/TT_N27+tlrc
...
foreach run ( $runs )
	...
	3dBrickStat -mask full_mask.$subj+tlrc -percentile 50 1 50 pb03.$subj.r${run}_e1.volreg+tlrc'[5]' > gms.1D
	set gms = `cat gms.1D`
	set gmsa = ($gms)
	...
	# Rescale to the large values that ICA prefers
 	foreach iEcho ( $echoes )
		...
		3dcalc -float -overwrite -a pb03.$subj.r${run}_e${iEcho}.volreg+tlrc \
		    -b rm.anatmask.bin+tlrc -expr "a*b*10000/${p50}" -prefix pb04.$subj.r${run}_e${iEcho}.in.nii.gz
	end		

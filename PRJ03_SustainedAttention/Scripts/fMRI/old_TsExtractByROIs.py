...
if __name__ == '__main__':
    ...
    if os.path.isfile(MaskPath):
        ...
        subprocess.call('3dROIstats -mask %s/Mask_EPIres_%s -nomeanout -nzvoxels -quiet %s/Mask_EPIres_%s > %sROIstats.1D' %
                (MaskDir,AtlasBase,MaskDir,AtlasBase,args.prefix),shell=True)
        ROIsize=np.loadtxt('%sROIstats.1D' % args.prefix)
        nROIs=ROIsize.shape[0]
        ...
        for t in range(1,nROIs+1):
            ...
            print('\n++ INFO: Computing SVD for ROI %s' % (t))
            # Use 3dmaskSVD to get 1st principal component rather than mean
            if int(ROIsize[t-1]) == 0:
                np.savetxt('tmp.ROI%s_TS.1D' % (t),np.zeros((nv,1)))
            else:
                subprocess.call('3dmaskSVD -vnorm -mask %s/Mask_EPIres_%s\"<%s..%s>\" %s > tmp.ROI%s_TS.1D' %
                    (MaskDir,AtlasBase[:-AtlasBase[::-1].index('.')-1],t,t,EPIPath,t),shell=True)
    # If NO mask is provided
        ...
        # Get number of ROIs and find empty ROIs
        subprocess.call('3dROIstats -mask %s/EPIres_%s -nomeanout -nzvoxels -quiet %s/EPIres_%s > %sROIstats.1D' %
                (MaskDir,AtlasBase,MaskDir,AtlasBase,args.prefix),shell=True)
        ROIsize=np.loadtxt('%sROIstats.1D' % args.prefix)
        nROIs=ROIsize.shape[0]
        ...
        for t in range(1,nROIs+1):
            ...
            if int(ROIsize[t-1]) == 0:
                ...
                subprocess.call('3dmaskSVD -vnorm -mask %s/EPIres_%s\"<%s..%s>\" %s > tmp.ROI%s_TS.1D' %
                    (AtlasDir,AtlasBase[:-AtlasBase[::-1].index('.')-1],t,t,EPIPath,t),shell=True)


    TmpFiles=(glob.glob("tmp.ROI*_TS.1D")).sort()
    for i in range(len(TmpFiles)):
        if os.stat(TmpFiles[i]) == 0:
            np.savetxt(TmpFiles[i],np.zeros((nv,1)))

    # Concatenate individual ROI timecourses into matrix and delete individual files
    subprocess.call('1dcat tmp.ROI*_TS.1D > %sROI_TS.1D' % (args.prefix),shell=True)
    subprocess.call('rm -f tmp.ROI*_TS.1D',shell=True)

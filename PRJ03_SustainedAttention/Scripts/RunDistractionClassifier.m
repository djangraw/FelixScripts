function [y_true,y_perm] = RunDistractionClassifier(subject,label1,label0,useEcho2,doPerms,fracTcVarToKeep,fracFcVarToKeep,doFMs,doPlots)
...
[~,AzLoo_bold,LRstats_bold,AzLto_bold] = RunSingleLR(permute(normfeats(isBoldFeat,isOkTrial),[1 3 2]),truth(isOkTrial),params);
if sum(isFcFeat)>0
    [~,AzLoo_fc,LRstats_fc,AzLto_fc] = RunSingleLR(permute(normfeats(isFcFeat,isOkTrial),[1 3 2]),truth(isOkTrial),params);
else
    fprintf('No FC features... classifier can''t run!\n')
    AzLoo_fc = 0;
    AzLto_fc = 0;
end
[~,AzLoo_eye,LRstats_eye,AzLto_eye] = RunSingleLR(permute(normfeats(isEyeFeat,isOkTrial),[1 3 2]),truth(isOkTrial),params);
[~,AzLoo_fmri,LRstats_fmri,AzLto_fmri] = RunSingleLR(permute(normfeats(isBoldFeat | isFcFeat,isOkTrial),[1 3 2]),truth(isOkTrial),params);
[~,AzLoo_all,LRstats_all,AzLto_all] = RunSingleLR(permute(normfeats(:,isOkTrial),[1 3 2]),truth(isOkTrial),params);
...
if doPerms    
    ...
    [Az_perm, y_perm, AzPairs_perm,Az_perm_LTO] = deal(cell(1,nPerms));
    for iPerm=1:nPerms
        fprintf('Perm %d/%d...\n',iPerm,nPerms);
        % permute truth
        ...
        % get results
        [~,AzLoo_bold,LRstats_bold,AzLto_bold] = RunSingleLR(permute(normfeats(isBoldFeat,isOkTrial),[1 3 2]),permtruth,params);
        [~,AzLoo_fc,LRstats_fc,AzLto_fc] = RunSingleLR(permute(normfeats(isFcFeat,isOkTrial),[1 3 2]),permtruth,params);
        [~,AzLoo_eye,LRstats_eye,AzLto_eye] = RunSingleLR(permute(normfeats(isEyeFeat,isOkTrial),[1 3 2]),permtruth,params);
        [~,AzLoo_fmri,LRstats_fmri,AzLto_fmri] = RunSingleLR(permute(normfeats(isBoldFeat | isFcFeat,isOkTrial),[1 3 2]),permtruth,params);
        [~,AzLoo_all,LRstats_all,AzLto_all] = RunSingleLR(permute(normfeats(:,isOkTrial),[1 3 2]),permtruth,params);
    ...
    fprintf('Done!\n')
    %% SAVE!
    save(sprintf('SBJ%02d_MultimodalClassifier_%s-%s_%s',subject,label1,label0,datestr(now,'YYYY-mm-DD')),'Az_true','y_true','AzPairs_true','Az_perm','Az_perm_LTO','y_perm','AzPairs_perm','subject','fcWinLength','TR','nFirstRemoved','fracTcVarToKeep','fracFcVarToKeep','labels','labelNames','label0','label1');
    ...
    % SAVE!
    save(sprintf('SBJ%02d_MultimodalClassifier_%s-%s_%s',subject,label1,label0,datestr(now,'YYYY-mm-DD')),'Az_true','y_true','AzPairs_true','subject','fcWinLength','TR','nFirstRemoved','fracTcVarToKeep','fracFcVarToKeep','label0','label1','Az_LTO');    

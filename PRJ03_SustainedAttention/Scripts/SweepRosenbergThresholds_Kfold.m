function [maskSizePos,maskSizeNeg,Rsq,p,r,p_spearman,r_spearman] = SweepRosenbergThresholds_Kfold(cp,cr,FC,fracCorrect,thresholds,doPlot)

% [maskSizePos,maskSizeNeg,Rsq,p,r,p_spearman,r_spearman] = SweepRosenbergThresholds_Kfold(cp,cr,FC,fracCorrect,thresholds,doPlot)
%
% INPUTS:
...
% Updated 2/5/17 by DJ - made into K-fold version (_Kfold)
...
[maskSizePos_subj,maskSizeNeg_subj] = deal(nan(nThresh,nFolds));
...
for i=1:nThresh
    ...
    for j=1:nFolds
        ...
        % Get FC match scores
        matchPos(i,testingSubj{j}) = GetFcTemplateMatch(FC(:,:,testingSubj{j}),isPos,[],[],'meanmult');
        matchNeg(i,testingSubj{j}) = GetFcTemplateMatch(FC(:,:,testingSubj{j}),isNeg,[],[],'meanmult');
        matchBoth(i,testingSubj{j}) = GetFcTemplateMatch(FC(:,:,testingSubj{j}),double(isPos)-double(isNeg),[],[],'meanmult');
    end

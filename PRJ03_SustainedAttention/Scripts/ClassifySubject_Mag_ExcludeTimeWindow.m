function [AzCv, fwdModelCv, statsCv, newParams, AzCvPerm, fwdModelCvPerm, statsCvPerm, yCv_allTrials] = ClassifySubject_Mag_ExcludeTimeWindow(subject,separationTimes,allParams)

% [AzCv, fwdModelCv, statsCv, newParams, AzCvPerm, fwdModelCvPerm, statsCvPerm, yCv_allTrials] = ClassifySubject_Mag_ExcludeTimeWindow(subject,separationTimes,allParams)
%
% Classify a single subject's data - with varied separation times between
% testing and training samples - using BOLD magnitude features.
%
...
% Updated 9/1/16 by DJ - added | magRuns~=magRuns(iTest) condition to iTrain
...
[Az,AzLoo,yCv,nTrain,nTrainInRun,nTrainInCond] = deal(nan(nTrials,nSepTimes));
...
for k=1:nSepTimes
    ...
    for j=1:nTrials % for each trial
        ...
        iTest = j;
        isTrain = abs(magTimes-magTimes(iTest))>separationTime | magRuns~=magRuns(iTest);
        ...
        nTrainNotInCond(j,k) = sum(isTrain & magRuns==magRuns(iTest) & magTruth~=magTruth(iTest));
        if doBalancedTrainingSet
            if nTrainInCond(j,k)>nTrainNotInCond(j,k)
                nToRemove = nTrainInCond(j,k) - nTrainNotInCond(j,k);
                iRemovable = find(isTrain & magRuns==magRuns(iTest) & magTruth==magTruth(iTest));
            ...
            if sum(isTrain & magRuns==magRuns(iTest))>0 && mean(magTruth(isTrain & magRuns==magRuns(iTest)))~=0.5
...
if nPerms>0
    ...
    for i=1:nPerms
        ...
        magTruthPerm = magTruth(randperm(numel(magTruth)));
        for k=1:nSepTimes
            separationTime = separationTimes(k);
            fprintf('---SeparationTime = %d...\n',separationTime);
            ...
            for j=1:nTrials % for each trial
                % find train and test data
                iTest = j;
                isTrain = abs(magTimes-magTimes(iTest))>separationTime;
%                 nTrain(j,k) = sum(isTrain);
%                 nTrainInRun(j,k) = sum(isTrain & magRuns==magRuns(iTest));
%                 nTrainInCond(j,k) = sum(isTrain & magRuns==magRuns(iTest) & magTruth==magTruth(iTest));

...
# RunSrtt3dDeconvolve_Filter.sh
#
# Regress out nuisance regressors like motion, band-pass filter, and WM/CSF timecourses.
#
# USAGE:
#   RunSrtt3dDeconvolve_Filter.sh $subj
# INPUTS:
...
3dDeconvolve -input pb05.$subj.r*.scale+tlrc.HEAD                            \
    ...
    -fout -tout                                                              \
    -x1D_stop                                                                \
    -fitts fitts.filter.${subj}                                              \
    -errts errts.filter.${subj}                                              \
    -bucket stats.filter.${subj}
...
mv stats.REML_cmd stats.filter.REML_cmd
tcsh -x stats.filter.REML_cmd
mv 3dREMLfit.err 3dREMLfit.filter.err # to avoid confusing with old version

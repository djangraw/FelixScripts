function [idx, pred_pos, pred_neg, pred_glm,masks] = RunKfoldBehaviorRegression_adam(FC,behav,thresh,corr_method,mask_method,nFolds)
...
% idx is the subjects without nans that were included in the analysis.
%
...
%%
%% added by adam 02/08/2018. Handling of nans in correlation matrix and behavior
%%

n_sub = size(FC,3); idx = ones(n_sub,1);
for i = 1:n_sub
    temp = FC(:,:,i);
    temp = temp(:);
  if sum(isnan(temp)) > 0
...
FC = FC(:,:,idx == 1); behav = behav(idx == 1);
disp(['++ Number of subject removed with NaNs: ' num2str(sum(idx))])
clear i ;
%%
% Set number of folds
%%
...
fprintf('Done!\n');
%% added mask output as structure rather than individual pieces.

         
masks = struct;
masks.pos = zeros(size(pos_mask_all));
masks.neg = zeros(size(neg_mask_all));
masks.pos_mask_all = pos_mask_all;
masks.neg_mask_all = neg_mask_all;

for i = 1:nFolds
  masks.pos(:,:,i) = pos_mask_all(:,:,i)+pos_mask_all(:,:,i)';
  masks.neg(:,:,i) = neg_mask_all(:,:,i)+neg_mask_all(:,:,i)';
end
masks.pos = sum(masks.pos,3);
masks.neg = sum(masks.neg,3);

function TestIscPValueTrends(subject,nRuns,maskFilename)

% TestIscPValueTrends(subject,nRuns)
%
% INPUTS: 
...
foo = load(sprintf('ISC_%s_%dRuns.mat',subject,nRuns(1)));
p = zeros([size(foo.pval), numel(nRuns)]);
for i=1:numel(nRuns)
    fprintf('loading %d/%d...\n',i,numel(nRuns));
    foo = load(sprintf('ISC_%s_%dRuns.mat',subject,nRuns(i)));
    p(:,:,:,i) = foo.pval;
end

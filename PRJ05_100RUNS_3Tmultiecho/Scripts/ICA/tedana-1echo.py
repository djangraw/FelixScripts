...
def tedpca(ste=0,pcrejfrac=0.3):
	# SINGLE ECHO: new input pcrejfrac specifies the chance every PC will have of being rejected.
	nx,ny,nz,ne,nt = catd.shape
	...
	#Compute K and Rho for PCA comps # SINGLE ECHO: LEAVE OUT K/RHO CALCS.
	"""
	...
	np.savetxt('sepca_mix.1D',v[ctb[:,1].argsort()[::-1],:].T)
	...
	"""
	# SINGLE ECHO: SELECT PC'S RANDOMLY
	pcscore = np.random.random(s.shape[0])
	pcsel = pcscore>pcrejfrac
...
def fitmodels_direct_1echo(catd,mmix,mask,t2s,tes,fout=None,reindex=False,mmixN=None,full_sel=True,debugout=False):
	...
	tsoc = np.array(optcom(catd,t2s,tes,mask),dtype=float)[mask] # SINGLE ECHO: should just return catd
	...
	# Calculate varex, which will still be important in comp selection
	for i in range(nc):
	...
	if reindex:
		...
		WTS = WTS[:,nnc]; tsoc_B=tsoc_B[:,nnc]
		# PSC=PSC[:,nnc]; tsoc_Babs=tsoc_Babs[:,nnc]
...
if __name__=='__main__':
	...
	parser.add_option('',"--pcrejfrac",dest='pcrejfrac',help="Fraction of PCs to remove (during random PC selection)",default='0.3')
	parser.add_option('',"--icvarexfrac",dest='icvarexfrac',help="Fraction of variance to remove (during random IC selection)",default='0.3')
	...
	if options.mixm!=None: 
		try:
			...
			pass
	pcrejfrac = float(options.pcrejfrac) # SINGLE ECHO: fraction of PCs to remove.
	...
	if options.mixm == None:
		...
		import mdp
		nc,dd = tedpca(options.ste,pcrejfrac) # SINGLE ECHO: MODIFIED TO SELECT COMPONENTS RANDOMLY

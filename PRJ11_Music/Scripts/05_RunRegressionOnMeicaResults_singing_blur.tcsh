...
#
# USAGE:
#   tcsh -xef 05_RunRegressionOnMeicaResults_singing_blur $subj $nRuns $echoTimes $outFolder 2>&1 | tee output.05_RunRegressionOnMeicaResults_blur.$subj.$outFolder
#
# INPUTS:
...
3dDeconvolve -input bl07.${subj}.r*.scale+tlrc.HEAD                           \
    ...
    -fout -tout -x1D bl.X.xmat.1D -xjpeg bl.X.jpg                                   \
    -x1D_uncensored bl.X.nocensor.xmat.1D                                        \
    -x1D_stop                                                                 \
    -errts bl.errts.$subj                                                        \
    -bucket bl.stats.$subj                                                       \
    -cbucket bl.cbucket.$subj                                                    \
    -jobs 32 \
    -overwrite
...
3dTproject -polort 0 -input bl07.$subj.r*.scale+tlrc.HEAD                    \
           ...
           -ort bl.X.nocensor.xmat.1D[0..${iLastNuisanceReg}] -overwrite -prefix bl.errts.${subj}.tproject
...
1d_tool.py -show_cormat_warnings -infile bl.X.xmat.1D |& tee bl.out.cormat_warn.txt

# -- execute the 3dREMLfit script, written by 3dDeconvolve --
# Rename and Run 3dREMLfit
mv bl.REML_cmd bl.stats.$subj.REML_cmd
tcsh -x bl.stats.$subj.REML_cmd

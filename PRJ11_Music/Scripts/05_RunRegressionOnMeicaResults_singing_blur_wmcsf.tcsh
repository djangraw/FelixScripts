#!/bin/tcsh -xef
# 05_RunRegressionOnMeicaResults_singing_blur
#
# USAGE:
#   tcsh -xef 05_RunRegressionOnMeicaResults_singing_blur_wmcsf.tcsh $subj $nRuns $echoTimes $outFolder 2>&1 | tee output.05_RunRegressionOnMeicaResults_blur_wmcsf.$subj.$outFolder
#
# INPUTS:
...
3dDeconvolve -input bl07.${subj}.r*.scale+tlrc.HEAD                           \
    ...
    -stim_times_AM1 27 ../stimuli/${subj}_Listen.1D 'dmBLOCK4(1)' -stim_label 27 listen \
    -fout -tout -x1D bl_wmcsf.X.xmat.1D -xjpeg bl_wmcsf.X.jpg                                   \
    -x1D_uncensored bl_wmcsf.X.nocensor.xmat.1D                                        \
    -x1D_stop                                                                 \
    -errts bl_wmcsf.errts.$subj                                                        \
    -bucket bl_wmcsf.stats.$subj                                                       \
    -cbucket bl_wmcsf.cbucket.$subj                                                    \
    -jobs 32 \
    -overwrite
...
3dDeconvolve -input bl07.${subj}.r*.scale+tlrc.HEAD                           \
    ...
    -stim_times_AM1 15 ../stimuli/${subj}_Improvise.1D 'dmBLOCK4(1)' -stim_label 15 improvise \
    -fout -tout -x1D bl_wmcsf.X.xmat.1D -xjpeg bl_wmcsf.X.jpg                                   \
    -x1D_uncensored bl_wmcsf.X.nocensor.xmat.1D                                        \
    -x1D_stop                                                                 \
    -errts bl_wmcsf.errts.$subj                                                        \
...
# Declare number of nuisance regressors, excluding stimulus regressors
set iLastNuisanceReg = 18 # total regressors + 5*nRuns (polorts) - 5 (stim regressors) - 1 (zero-based)
...
3dDeconvolve -input bl07.${subj}.r*.scale+tlrc.HEAD                               \
    ...
    -stim_times_AM1 99 ../stimuli/${subj}_Sing.1D 'dmBLOCK4(1)' -stim_label 99 sing \
    -stim_times_AM1 100 ../stimuli/${subj}_Speak.1D 'dmBLOCK4(1)' -stim_label 100 speak \
    -stim_times_AM1 101 ../stimuli/${subj}_Imagine.1D 'dmBLOCK4(1)' -stim_label 101 imagine \
    -stim_times_AM1 102 ../stimuli/${subj}_Rest.1D 'dmBLOCK4(1)' -stim_label 102 rest \
...
# Declare number of nuisance regressors, excluding stimulus regressors
set iLastNuisanceReg = 138 # total regressors + 5*nRuns (polorts) - 5 (stim regressors) - 1 (zero-based)
...
3dTproject -polort 0 -input bl07.$subj.r*.scale+tlrc.HEAD                    \
           ...
           -ort bl_wmcsf.X.nocensor.xmat.1D[0..${iLastNuisanceReg}] -overwrite -prefix bl_wmcsf.errts.${subj}.tproject
...
1d_tool.py -show_cormat_warnings -infile bl_wmcsf.X.xmat.1D |& tee bl_wmcsf.out.cormat_warn.txt

# -- execute the 3dREMLfit script, written by 3dDeconvolve --
# Rename and Run 3dREMLfit
mv bl_wmcsf.REML_cmd bl_wmcsf.stats.$subj.REML_cmd
tcsh -x bl_wmcsf.stats.$subj.REML_cmd

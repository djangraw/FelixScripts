#!/bin/tcsh -xef
# 05_RunRegressionOnMeicaResults_singing_AddOC
#
# USAGE:
#   tcsh -xef 05_RunRegressionOnMeicaResults_singing_AddOC.tcsh $subj $nRuns $echoTimes $outFolder 2>&1 | tee output.05_RunRegressionOnMeicaResults_singing_AddOC.$subj.$outFolder
#
# INPUTS:
...
# Updated 5/1/17 by DJ - modified to _AddOC version
...
# ================================== COPY ==================================
# Retrieve and relabel MEICA's OC time series
foreach run ( $runs )
    ...
    3dresample -orient LPI -prefix ${output_dir}/oc05.$subj.r${run}.optcom.nii -overwrite -inset ${output_dir}/TED.$subj.r${run}/ts_OC.nii
  	# specify that the denoised timeseries is in MNI atlas space
  	3drefit -space MNI ${output_dir}/oc05.$subj.r${run}.optcom.nii
...
foreach run ( $runs )
    ...
    3dTstat -overwrite -prefix rm.mean.r${run}_oc oc05.${subj}.r${run}.optcom.nii
    # Scale
    3dcalc -a oc05.${subj}.r${run}.optcom.nii -b rm.mean.r${run}_oc+tlrc \
        -expr 'min(200, a/b*100) *step(a)*step(b)' \
            -overwrite -prefix oc06.${subj}.r${run}.scale
end
...
3dDeconvolve -input oc06.${subj}.r*.scale+tlrc.HEAD                           \
    ...
    -fout -tout -x1D oc.X.xmat.1D -xjpeg oc.X.jpg                                   \
    -x1D_uncensored oc.X.nocensor.xmat.1D                                        \
    -x1D_stop                                                                 \
    -errts oc.errts.$subj                                                        \
    -bucket oc.stats.$subj                                                       \
    -cbucket oc.cbucket.$subj                                                    \
    -jobs 32 \
    -overwrite
...
# Declare number of nuisance regressors, excluding stimulus regressors
set iLastNuisanceReg = 135 # total regressors + 5*nRuns (polorts) - 5 (stim regressors) - 1 (zero-based)

endif

# -- use 3dTproject to project out regression matrix WITHOUT STIM REGRESSORS --
3dTproject -polort 0 -input oc06.$subj.r*.scale+tlrc.HEAD                    \
           -censor censor_${subj}_combined_2.1D -cenmode ZERO                 \
           -ort oc.X.nocensor.xmat.1D[0..${iLastNuisanceReg}] -overwrite -prefix oc.errts.${subj}.tproject
...
1d_tool.py -show_cormat_warnings -infile oc.X.xmat.1D |& tee oc.out.cormat_warn.txt

# -- execute the 3dREMLfit script, written by 3dDeconvolve --
# Rename and Run 3dREMLfit
mv oc.REML_cmd oc.stats.$subj.REML_cmd
tcsh -x oc.stats.$subj.REML_cmd

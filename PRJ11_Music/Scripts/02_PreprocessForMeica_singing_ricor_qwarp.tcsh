#!/bin/tcsh -xef
# 02_PreprocessForMeica_singing_ricor_qwarp.tcsh
#
# USAGE:
#   tcsh -xef 02_PreprocessForMeica_singing_ricor_qwarp.tcsh $subj $nRuns $echoTimes $outFolder 2>&1 | tee output.02_PreprocessForMeica.$subj.$outFolder
#
# INPUTS:
...
# Updated 5/1-17 by DJ - ricor
# Update 5/21/17 by DJ - switched to qwarp
...
if ( $SKIP_EARLY_STUFF == 0 ) then
  ...
  # 3dbucket -prefix vr_base pb03.$subj.r01.tshift+orig"[2]"
  ...
  3dZeropad -S 10 -I 50 -prefix rm.${subj}_Anat_bc_ns_al_clip_padded -overwrite ${subj}_Anat_bc_ns_al_clip+orig
  # convert matrix
  cat_matvec ${subj}_Anat_bc_ns_al_clip_mat.aff12.1D > rm.${subj}_Anat_bc_ns_al_clip_mat_3x4.aff12.1D
  # warp anatomy to epi with no clipping
  3dWarp -matvec_out2in rm.${subj}_Anat_bc_ns_al_clip_mat_3x4.aff12.1D -gridset rm.${subj}_Anat_bc_ns_al_clip_padded+orig -prefix ${subj}_Anat_bc_ns_al_keep -overwrite ${subj}_Anat_bc_ns+orig
...
# auto_warp.py -base MNI_caez_N27+tlrc -input     \
#              ${subj}_Anat_bc_ns_al_keep+orig \
...
3dbucket -prefix ${subj}_Anat_bc_ns_al_keep  \
    awpy/${subj}_Anat_bc_ns_al_keep.aw.nii*
mv awpy/anat.un.aff.Xat.1D .
mv awpy/anat.un.aff.qw_WARP.nii .

# Apply to clipped dataset
3dNwarpApply -master ${subj}_Anat_bc_ns_al_keep+tlrc -dxyz 3                   \
             -source ${subj}_Anat_bc_ns_al_clip+orig                         \
             -nwarp "anat.un.aff.qw_WARP.nii"     \
             -prefix ${subj}_Anat_bc_ns_al_clip+tlrc
...
if ( ! -f ${subj}_Anat_bc_ns_al_keep+tlrc.HEAD ) then
    echo "** missing +tlrc warp dataset:                                  \
        ${subj}_Anat_bc_ns_al_keep+tlrc.HEAD"
    exit
endif
...
foreach run ( $runs )
   ...
   cat_matvec -ONELINE                                                       \
	...
	foreach iEcho ( $echoes )
      ...
      3dNwarpApply -master ${subj}_Anat_bc_ns_al_keep+tlrc -dxyz 3                   \
                   -source pb02p1.$subj.r${run}_e${iEcho}.tshift+orig                         \
                   -nwarp "anat.un.aff.qw_WARP.nii mat.r$run.warp.aff12.1D"     \
...
3dNwarpApply -master ${subj}_Anat_bc_ns_al_keep+tlrc -dxyz 3                       \
             -source external_volreg_base+orig                                             \
             -nwarp "anat.un.aff.qw_WARP.nii mat.basewarp.aff12.1D"           \
             -prefix final_epi_vr_base

# create an anat_final dataset, aligned with stats
3dcopy -overwrite ${subj}_Anat_bc_ns_al_keep+tlrc anat_final.$subj
...
3dNwarpApply -source ${subj}_Anat_bc_ns+orig                               \
...
3dfractionize -input ${subj}_Anat_bc_ns_al_clip+tlrc -template pb03.$subj.r${runs[1]}_e${iRegEcho}.volreg+tlrc -clip 0.5 -overwrite -prefix rm.anatmask # resample skull-stripped, epi-aligned anat file to match epi resolution

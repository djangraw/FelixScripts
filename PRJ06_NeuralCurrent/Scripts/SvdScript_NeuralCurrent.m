...
% Updated 8/17/15 by DJ - switched to neural current's e3-1 input.

% Load data
datadir = '/Users/jangrawdc/Documents/PRJ06_NeuralCurrent/PrcsData/SBJ01/';
filename = 'SBJ01_e1m3_103runs.bucket_REML+orig';
cd(datadir)
Opt = struct('Frames',2:31);
[err, brick, Info, ErrMessage] = BrikLoad (filename,Opt);
...
%% Plot ALL components
nCompsPerPlot = 6;
nPlots = ceil(size(V,2)/nCompsPerPlot);
nRows = ceil(sqrt(nPlots));
nCols = ceil(nPlots/nRows);
figure(22); clf;
for i=1:nPlots
    subplot(nRows,nCols,i);
    iComps = (1:nCompsPerPlot)+nCompsPerPlot*(i-1);
    plot(V(:,iComps));
    hold on;
    plot(get(gca,'xlim'),[0 0],'k:')
    xlabel('time (samples)')
    ylabel('activity (AU)');
    legendstr = cell(1,nCompsPerPlot);
    for j=1:nComps, legendstr{j}=sprintf('PC #%d',iComps(j)); end;
    legend(legendstr);
end
...
%% SAVE RESULTS
save('SBJ01_e1m3Mean_SVD_CorrCoef.mat','filename','brick','Info','S','V','match','T','filtersize','brickmed','nComps','legendstr');

%% LOAD RESULTS
load SBJ01_e1m3Mean_SVD_CorrCoef
...
%% NEW 8/15
%% Run GLM with neural-current-ish component (IN PROGRESS)
bricksize = size(brick);
match = nan([bricksize(1:3),nComps]);
amp = nan(bricksize(1:3));
disp('Getting Correlation Coefficients...');
